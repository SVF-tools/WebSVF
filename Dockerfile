...
RUN apt-get install -y curl git nodejs npm lsb-core net-tools cmake sudo wget build-essential libtinfo-dev libtinfo5 python3-pip libgraphviz-dev graphviz
...
CMD code-server --install-extension tianyangguan.svftools --force && code-server --auth="none" --host 0.0.0.0 --port 8080
