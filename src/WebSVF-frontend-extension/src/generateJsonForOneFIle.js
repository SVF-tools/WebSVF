...
module.exports = {
    generateJSONForOneFile: generate
}

function generate(dir, svfPath, callback) {
    var command = "clang -c -emit-llvm -g " + dir + " -o example.bc";
    createBcFile(command, function() {
        generateJSON(dir, svfPath, callback);
    })
}

function createBcFile(command, callback) {
    exec(command, (err, stdout, stderr) => {
        if(err) {
            throw new Error('Create bytefile failed.');
        }
        callback();
    });
}

function generateJSON(dir, svfPath, callback) {
    ...
    bcFilesList.forEach(element => {
        ...
        exec(commend, (err, stdout, stderr) => {
            if (stderr != "" && !(stderr.indexOf("failed") != -1)) {
                analyzeData(stderr.toString(), projectPath, allFilesList, callback);
            }
            exec('rm example.bc');
        });
    });
...
function analyzeData(output, projectPath, allFilesList, callback) {
    //console.log(output);
    var bugreports;
    var bugreportsArray = [];
    var errors = [];
