...
This extension can detect whether the **Bug Analysis Tool** is missing. **Bug Analysis Tool** is also called **WebSVF frontend server**, which is downloaded by the **WebSVF Backend script**. If missing, the frontend extension will download the ``required files and tools`` in a compressed package format ``zip``, and then uncompress the ``zip`` locally to a **specific hidden directory** (the directory is consistent with the address downloaded by the WebSVF Backend script) firstly and then load the **Bug Analysis Tool** again. 
