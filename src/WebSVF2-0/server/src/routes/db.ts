...
const db: IRoutesFactory = (app: Express) => {
  app.post('/db/saveFile/', async (req, res) => {
    ...
    //If the project is found (core logic for this endpoint)
      ...
      const foundFolder = await usersCollection.exists({
        googleID: 'abcd',
        'projects.projectID': 'abc',
      ...
      if (foundFolder) {
        ...
        const foundFile = await usersCollection.exists({
          googleID: 'abcd',
          'projects.projectID': 'abc',
          'projects.userCode.folderName': req.body.folderName,
        ...
        if (foundFile) {
          ...
          const file = await usersCollection.findOne({
            googleID: 'abcd',
            'projects.projectID': 'abc',
            'projects.userCode.folderName': req.body.folderName,
            'projects.userCode.files.fileName': req.body.fileName
          ...
          const codeFolderIndex = file.projects[projectIndex].userCode.findIndex((code) => {
            return code.folderName === req.body.folderName;
          });

          const codeFileIndex = file.projects[projectIndex].userCode[codeFolderIndex].files.findIndex((code) => {
          ...
          const newVersion = (Number(file.projects[projectIndex].userCode[codeFolderIndex].files[codeFileIndex].version) + 0.01).toFixed(2);
          ...
          await usersCollection.findOneAndUpdate(
            ...
            {
              arrayFilters: [{ 'i.projectID': 'abc' }, { 'j.folderName': req.body.folderName }, { 'k.fileName': req.body.fileName }]
            }
          );
        ...
        //If the file doesn't exist in the db
          ...
          await usersCollection.findOneAndUpdate(
            ...
            {
              arrayFilters: [{ 'h.projectID': 'abc' }, { 'i.folderName': req.body.folderName }],
              upsert: true
            }
          );
  ...
  app.get('/db/getFiles', async (req, res) => {
    ...
    const projectExists = await usersCollection.exists({
      googleID: 'abcd',
      'projects.projectID': 'abc'
    });
    if (projectExists) {
      //Find and send the projects array of the current user
      const project = await usersCollection.findOne({
        googleID: 'abcd',
        'projects.projectID': 'abc'
      });
