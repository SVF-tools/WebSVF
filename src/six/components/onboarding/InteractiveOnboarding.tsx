...
const InteractiveOnboarding: React.FC<InteractiveOnboardingProps> = ({
  ...
  const currentStepContent = currentStepData.subSteps && currentStepData.subSteps.length > 0
    ? currentStepData.subSteps[currentSubStep]
    : null;
  
  const hasSubSteps = !!(currentStepData.subSteps && currentStepData.subSteps.length > 0);
  const isInSubStep = hasSubSteps && currentSubStep > 0;
  ...
  const effectiveAllowInteraction = currentStepContent?.allowInteraction ?? currentStepData.allowInteraction;
  ...
  const effectiveRequireOutput = currentStepContent?.requireOutput ?? (!hasSubSteps || currentSubStep === 0 ? currentStepData.requireOutput : undefined);
  const forcePreferredSide = currentStepContent?.forcePreferredSide ?? currentStepData.forcePreferredSide;
  ...
  const calculatePosition = useCallback(() => {
    ...
    const shouldPadHighlight = effectiveTargetElement === '#submit-codeBar-compile-options-container';
    ...
    const spaceTop = rect.top;
      ...
      if (preferredPosition === 'center' || !preferredPosition) {
        ...
        side = 'right';
      } else if (forcePreferredSide && (preferredPosition === 'right' || preferredPosition === 'left' || preferredPosition === 'top' || preferredPosition === 'bottom')) {
        ...
        if (preferredPosition !== 'auto' && (['right','left','bottom','top'] as string[]).includes(preferredPosition)) {
          order = [preferredPosition as 'right' | 'left' | 'bottom' | 'top', ...sides.filter((s) => s !== preferredPosition)];
        }
      ...
      if (side === 'right' || side === 'left') {
        ...
        if (overlapsY) {
          if (spaceBottom >= spaceTop) {
            ...
            top = Math.max(
              rect.top - safeGap - tooltipHeight / 2,
        ...
        if (overlapsX) {
          if (spaceRight >= spaceLeft) {
            ...
            left = Math.max(
              rect.left - safeGap - tooltipWidth / 2,
      ...
      const overlapsTarget = () => {
        ...
        return !(box.right + safeGap <= rect.left || box.left - safeGap >= rect.right || box.bottom + safeGap <= rect.top || box.top - safeGap >= rect.bottom);
      };
      ...
      // Absolute final override: never render as bottom; flip to top if still bottom
      if ((side as any) === 'bottom') {
        side = 'top';
        top = Math.max(padding, rect.top - tooltipHeight - Math.max(arrowSize, safeGap));
        left = rect.left + rect.width / 2;
        // clamp after override
        top = Math.max(padding, Math.min(top, window.innerHeight - padding - tooltipHeight));
        left = Math.max(
          padding + tooltipWidth / 2,
          Math.min(left, window.innerWidth - padding - tooltipWidth / 2)
    ...
    setTooltipSide(side);
  }, [currentStepData, effectiveTargetElement]);
  ...
  }, [currentStep, currentStepData.subSteps?.length]);
  ...
  const desired = currentStepContent?.desiredOutput || currentStepData.desiredOutput;
    if (desired) {
      ...
      const hasClickRequirement = !!(currentStepContent?.requireClick ?? currentStepData.requireClick);
      if (cameFromPrev || !hasClickRequirement) {
        try {
          ...
          document.dispatchEvent(new CustomEvent('websvf-tutorial-set-output', { detail: { output: desired } }));
  ...
  useEffect(() => {
    ...
    if (element) {
      ...
      const isHuge = r.height >= window.innerHeight * 0.9 || r.top <= 0 || r.bottom >= window.innerHeight;
      if (!isHuge) {
  ...
  useEffect(() => {
    ...
    const isNavbarTarget = effectiveTargetElement && 
  ...
  useEffect(() => {
    ...
    const handleClick = (e: MouseEvent) => {
      ...
      const sendButtonOnly =
        ...
        !!effectiveClickMessage && /send button/i.test(effectiveClickMessage);
      if (sendButtonOnly) {
  ...
  }, [isOpen, effectiveRequireClick, effectiveTargetElement, effectiveClickMessage, currentStepData.waitForAction]);
  ...
  useEffect(() => {
    ...
    const targetIsOutputMenu = !!effectiveTargetElement && effectiveTargetElement.includes('.output-menu-bar');
  ...
  }, [isOpen, currentStep, currentSubStep, currentOutput, effectiveRequireOutput, effectiveRequireClick, currentStepData.requireMultipleClicks, currentStepData.waitForAction]);
  ...
  const modalOpen = typeof document !== 'undefined' && document.documentElement.dataset.onboardingModalOpen === 'true';
  ...
  const targetIsNavbar = !!effectiveTargetElement && (
    effectiveTargetElement.includes('#six-navbar') ||
    effectiveTargetElement.includes('#settings-icon') ||
    effectiveTargetElement.includes('#share-icon') ||
    effectiveTargetElement.includes('#help-icon') ||
  ...
  const renderSide: 'top' | 'bottom' | 'left' | 'right' = effectiveTargetElement ? (targetIsNavbar ? 'bottom' : 'top') : tooltipSide;
  const renderTop = (() => {
  ...
  const renderLeft = (() => {
    ...
    if (!targetIsNavbar) {
      ...
      const overlapsX = !(xRight + safeGap <= highlightRect.left || xLeft - safeGap >= highlightRect.right);
      ...
      if (overlapsX) {
        ...
        if (spaceRight >= spaceLeft) {
          ...
          center = Math.max(
            padding + ttW / 2,
            highlightRect.left - safeGap - ttW / 2
          );
        }
      }
    }
    ...
    <div className="interactive-onboarding-overlay">
        ...
        <div className="interactive-onboarding-backdrop">
          {highlightRect ? (
            <>
              ...
              {highlightLayerRef.current && createPortal(
                <div
      ...
      {!modalOpen && (
        ...
        <div className="tooltip-content" style={{ display: 'block', visibility: 'visible', opacity: 1 }}>
          <h3 className="tooltip-title" style={{ color: 'inherit', display: 'block', visibility: 'visible', opacity: 1 }}>
            {currentStepData.title}
          </h3>
          <div className="tooltip-description" style={{ color: 'inherit', display: 'block', visibility: 'visible', opacity: 1 }}>
            {effectiveContent}
          </div>
          
          {/* Show click requirement message */}
          {effectiveRequireClick && !clickedElements.has(effectiveTargetElement || '') && !(effectiveRequireOutput && currentOutput !== effectiveRequireOutput) && (
          ...
          {effectiveRequireClick && currentStepData.waitForAction && clickedElements.has(effectiveTargetElement || '') && !actionCompleted && (
            <div className="tooltip-requirement">
          ...
          {currentStepData.requireMultipleClicks && (
            <div className="tooltip-requirement">
              Clicked: {Array.from(clickedElements).filter(sel => 
                currentStepData.requireMultipleClicks?.includes(sel)
              ).length} / {currentStepData.requireMultipleClicks.length}
            </div>
          )}
        ...
        <div className="tooltip-navigation">
          ...
          <button className="tooltip-btn tooltip-btn-primary" onClick={handleNext} disabled={(() => {
            ...
            if (currentStepData.requireMultipleClicks) {
              const allClicked = currentStepData.requireMultipleClicks.every(selector => clickedElements.has(selector));
      ...
      {showSkipConfirm && (
        <div className="skip-confirm-overlay">
          <div className="skip-confirm-modal">
            <h3>Skip Tutorial?</h3>
            <p>You can replay this tutorial anytime by clicking the help button (?) in the navbar.</p>
