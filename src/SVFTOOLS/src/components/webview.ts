...
class WebPanel {
    ...
    protected createWebviewPanel(
        ...
        this.setWebpageWithSvg(webPanel, webInfo.homeRelativePath, webInfo.svgRelativePath, context);
        return webPanel;
    }
    ...
    protected setWebpageWithSvg(
        webPanel: vscode.WebviewPanel,
        relativePath: string,
        svgPath: string,
        context: vscode.ExtensionContext
    ) {
        webPanel.webview.html = this.conventHtmlAddSvg(relativePath, svgPath, context);
    }
    /**
    ...
    protected conventHtml(
        ...
        html = html.replace(
            /(<link.+?href="|<script.+?src="|<img.+?src="|<embed.+src="|<iframe.+src="|import.+?from ")(.+?)"/g,
            (m, $1, $2) => {
