    ...
    for (let i = 0; i < string1.length; i++) {
        ...
        if(i==0) {
            ...
            if(err_type==='Syntax'){
                ...
                codeStringSerialised += `<span id="err${error_tracker}" class="alert-logical nocode"><button id="${error_tracker}*!*err" onclick="add_errLines(${error_tracker})" class="err-button">${err_name}</button>\n<a>${err_descr}</a></span><a class="code-body" id="${(i+1)}">${string1[i]}</a>\n`;
                if(!$.isEmptyObject(jsonString.bugreport[file_tracker].Errors[error_tracker].StackTrace)){
                    ll_stacktrace['ln'] = (i+1);
                    ll_stacktrace['type'] = err_type;
                    ll_stacktrace['StackTrace'] = jsonString.bugreport[file_tracker].Errors[error_tracker].StackTrace;
                ...
                codeStringSerialised += `<span id="err${error_tracker}" class="alert-semantic nocode"><button id="${error_tracker}*!*err" onclick="add_errLines(${error_tracker})" class="err-button">${err_name}</button>\n<a>${err_descr}</a></span><a class="code-body" id="${(i+1)}">${string1[i]}</a>\n`;
                if(!$.isEmptyObject(jsonString.bugreport[file_tracker].Errors[error_tracker].StackTrace)){
                    ll_stacktrace['ln'] = (i+1);
                    ll_stacktrace['type'] = err_type;
                    ll_stacktrace['StackTrace'] = jsonString.bugreport[file_tracker].Errors[error_tracker].StackTrace;
                ...
                codeStringSerialised += `<span id="err${error_tracker}" class="alert nocode"><button id="${error_tracker}*!*err" onclick="add_errLines(${error_tracker})" class="err-button">${err_name}</button>\n<a>${err_descr}</a></span><a class="code-body" id="${(i+1)}">${string1[i]}</a>\n`;
                if(!$.isEmptyObject(jsonString.bugreport[file_tracker].Errors[error_tracker].StackTrace)){
                    ll_stacktrace['ln'] = (i+1);
                    ll_stacktrace['type'] = err_type;
                    ll_stacktrace['StackTrace'] = jsonString.bugreport[file_tracker].Errors[error_tracker].StackTrace;
